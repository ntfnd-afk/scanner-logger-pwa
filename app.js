...
async function loadSettings(){ const rows=await getAll(APP.db,'settings'); rows.forEach(r=> APP.state[r.key]=r.value); $('#operatorInput').value=APP.state.operator||''; $('#syncUrlInput').value=APP.state.syncUrl||''; $('#sendPlainInput').checked=APP.state.sendPlain!==false; applyTheme(APP.state.theme||'dark'); $('#themeToggle').checked=(APP.state.theme==='light'); }
...
async function onScan(code){
  ...
  if(t==='CITY'){
    if(/^CITY:CLOSE$/i.test(code)){
      ...
      APP.state.city=null; say('CITY_CLOSED'); setStatePill('ok','IDLE'); render(); return;
    }
    ...
    APP.state.city=code.substring(5); setStatePill('ok','CITY'); await logEvent({type:'CITY',code:APP.state.city}); render();
  } else if(t==='BOX'){
    if(!APP.state.city){ setStatePill('warn','NO CITY'); say('NO_CITY'); await logEvent({type:'ERROR',code:'NO_CITY'}); render(); return; }
    ...
    if(APP.state.box && APP.state.box===val){
      ...
      APP.state.box=null; APP.state.client=null; APP.state.itemsInBox=0; APP.state.boxStart=null; setStatePill('ok','IDLE'); render();
    } else {
      if(APP.state.box){
      ...
      const [client]=val.split('/'); APP.state.client=client||''; APP.state.box=val; APP.state.boxStart=Date.now(); APP.state.itemsInBox=0; APP.state.lastBoxItemsCount=0; await logEvent({type:'BOX',code:val}); setStatePill('ok','BOX'); render();
    }
  } else {
    if(!APP.state.box){ setStatePill('warn','NO BOX'); say('NO_BOX'); await logEvent({type:'ERROR',code:'NO_BOX'}); render(); return; }
    APP.state.itemsInBox+=1; await logEvent({type:'ITEM',code}); render(); updateItemsIndicator(); scheduleDebouncedSync();
  }
}
